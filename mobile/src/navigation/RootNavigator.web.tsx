...
/**
 * Web: imports estáticos (sin React.lazy).
 * Metro async chunks en web rompían con "Requiring unknown module" tras deploys parciales.
 */
...
function RoleNavigator({ role }: { role: string }) {
  ...
  if (role === 'restaurant') return <RestaurantNavigator />;
...
function MainRoutes() {
  ...
  if (!user && !isGuest) {
    const resetToken = getWebResetToken();
    return (
      <AuthStack.Navigator
